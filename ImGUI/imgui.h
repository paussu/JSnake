...
// (we generally use UTF-8 encoded string in the API. This is storage specifically for a decoded character used for keyboard input and mDisplay)
...
namespace ImGui
{
    ...
    IMGUI_API void          ShowAboutWindow(bool* p_open = NULL);       // create About window. mDisplay Dear ImGui version, credits and build/system information.
    IMGUI_API void          ShowMetricsWindow(bool* p_open = NULL);     // create Metrics/Debugger window. mDisplay Dear ImGui internals: windows, draw commands, various internal state, etc.
    ...
    IMGUI_API void          LabelText(const char* label, const char* fmt, ...)              IM_FMTARGS(2); // mDisplay text+label aligned the same way as value+label widgets
    ...
    // - Adjust format string to decorate the value with a prefix, a suffix, or adapt the editing and mDisplay precision e.g. "%.3f" -> 1.234; "%5.2f secs" -> 01.23 secs; "Biscuit: %.0f" -> Biscuit: 1; etc.
    ...
    //   If you get a warning converting a float to ImGuiSliderFlags, read https://github.com/ocornut/imgui/issues/3361
    IMGUI_API bool          SliderFloat(const char* label, float* v, float v_min, float v_max, const char* format = "%.3f", ImGuiSliderFlags flags = 0);     // adjust format to decorate the value with a prefix or a suffix for in-slider labels or unit mDisplay.
    ...
    IMGUI_API bool          ColorButton(const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags = 0, ImVec2 size = ImVec2(0, 0)); // mDisplay a color square/button, hover for details, return true when pressed.
    ...
    IMGUI_API bool          CollapsingHeader(const char* label, bool* p_open, ImGuiTreeNodeFlags flags = 0); // when 'p_open' isn't NULL, mDisplay an additional small close button on upper right of the header
    IMGUI_API void          SetNextItemOpen(bool is_open, ImGuiCond cond = 0);                  // set next TreeNode/CollapsingHeader open state.

    // Widgets: Selectables
    // - A selectable highlights when hovered, and can mDisplay another color when selected.
    ...
    IMGUI_API void          LogButtons();                                                       // helper to mDisplay buttons for logging to tty/file/clipboard
    IMGUI_API void          LogText(const char* fmt, ...) IM_FMTARGS(1);                        // pass text data straight to log (without being displayed)

    // Drag and Drop
    // - [BETA API] API may evolve!
    // - If you stop calling BeginDragDropSource() the payload is preserved however it won't have a preview tooltip (we currently mDisplay a fallback "..." tooltip as replacement)
    ...
    IMGUI_API const char*   GetStyleColorName(ImGuiCol idx);                                    // get a string corresponding to the enum value (for mDisplay, saving, etc.).
...
enum ImGuiInputTextFlags_
{
    ...
    ImGuiInputTextFlags_Password            = 1 << 15,  // Password mode, mDisplay all characters as '*'
...
enum ImGuiSelectableFlags_
{
    ...
    ImGuiSelectableFlags_Disabled           = 1 << 3,   // Cannot be selected, mDisplay grayed out text
...
enum ImGuiDragDropFlags_
{
    ...
    ImGuiDragDropFlags_SourceNoPreviewTooltip       = 1 << 0,   // By default, a successful call to BeginDragDropSource opens a tooltip so you can mDisplay a preview or description of the source contents. This flag disable this behavior.
...
enum ImGuiColorEditFlags_
{
    ...
    ImGuiColorEditFlags_NoLabel         = 1 << 7,   //              // ColorEdit, ColorPicker: disable mDisplay of inline text label (the label is still forwarded to the tooltip and picker).
    ...
    ImGuiColorEditFlags_AlphaPreview    = 1 << 17,  //              // ColorEdit, ColorPicker, ColorButton: mDisplay preview as a transparent color over a checkerboard, instead of opaque.
    ImGuiColorEditFlags_AlphaPreviewHalf= 1 << 18,  //              // ColorEdit, ColorPicker, ColorButton: mDisplay half opaque / half checkerboard, instead of opaque.
...
enum ImGuiSliderFlags_
{
    ...
    ImGuiSliderFlags_NoRoundToFormat        = 1 << 6,       // Disable rounding underlying value to match precision of the mDisplay format string (e.g. %.3f values are rounded to those 3 digits)
...
// User code may request backend to mDisplay given cursor by calling SetMouseCursor(), which is why we have some cursors that are marked unused here
...
struct ImGuiStyle
{
    ...
    ImVec2      DisplayWindowPadding;       // Window position are clamped to be visible within the mDisplay area or monitors by at least this amount. Only applies to regular windows.
...
struct ImGuiIO
{
    ...
    ImVec2      DisplaySize;                    // <unset>          // Main mDisplay size, in pixels.
    ...
    ImVec2      DisplayFramebufferScale;        // = (1, 1)         // For retina mDisplay or other situations where window coordinates are different from framebuffer coordinates. This generally ends up in ImDrawData::FramebufferScale.
    ...
    bool        WantTextInput;                  // Mobile/console: when set, you may mDisplay an on-screen keyboard. This is set by Dear ImGui when it wants textual keyboard input to happen (e.g. when a InputText widget is active).
...
// - Clipper calculate the actual range of elements to mDisplay based on the current clipping rectangle, position the cursor before the first visible element.
...
struct ImDrawList
{
    ...
    // - "uv_min" and "uv_max" represent the normalized texture coordinates to use for those corners. Using (0,0)->(1,1) texture coordinates will generally mDisplay the entire texture.
...
struct ImDrawData
{
    ...
    ImVec2          FramebufferScale;       // Amount of pixels for each unit of DisplaySize. Based on io.DisplayFramebufferScale. Generally (1,1) on normal mDisplay, (2,2) on OSX with Retina mDisplay.
...
struct ImFontAtlasCustomRect
{
    ...
    ImVec2          GlyphOffset;    // Input    // For custom font glyphs only: glyph mDisplay offset
