...
#endif
#define IM_TABSIZE                      (4)
#define IM_F32_TO_INT8_UNBOUND(_VAL)    ((int)((_VAL) * 255.0f + ((_VAL)>=0 ? 0.5f : -0.5f)))   // Unsaturated, for mDisplay purpose
...
struct IMGUI_API ImRect
{
    ...
    void        ClipWith(const ImRect& r)           { Min = ImMax(Min, r.Min); Max = ImMin(Max, r.Max); }                   // Simple version, may lead to an inverted rectangle, which is fine for Contains/Overlaps test but not for mDisplay.
...
struct ImGuiContext
{
    ...
    // Windows state
    ImVector<ImGuiWindow*>  Windows;                            // Windows, sorted in mDisplay order, back to front
...
struct IMGUI_API ImGuiWindowTempData
{
    ...
    ImRect                  LastItemDisplayRect;    // End-user mDisplay rect for last item (only valid if LastItemStatusFlags & ImGuiItemStatusFlags_HasDisplayRect)
...
struct IMGUI_API ImGuiWindow
{
    ...
    bool                    Hidden;                             // Do not mDisplay (== HiddenFrames*** > 0)
    ...
    ImGuiWindow*            RootWindowForTitleBarHighlight;     // Point to ourself or first ancestor which will mDisplay TitleBgActive color when this window is active.
